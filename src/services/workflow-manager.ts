...
export async function moveToDrafts(filepath: string, filename: string, config: ResumateConfig): Promise<string> {
  const dest = path.join(config.draftsDir, filename);
  await moveFile(filepath, dest);
  return dest;
}

export async function moveToInProgress(filepath: string, filename: string, config: ResumateConfig): Promise<string> {
  const dest = path.join(config.inProgressDir, filename);
  await moveFile(filepath, dest);
  return dest;
}

export async function moveToArchive(filepath: string, filename: string, config: ResumateConfig): Promise<string> {
